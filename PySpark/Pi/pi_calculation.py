...
# If you got error Unable to import 'pyspark.sql''
# You need to run "pip install pyspark" in the cloudshell terminal
...
def calculate_pi(partitions, output_uri):
    ...
    :param output_uri: The URI where the output is written, such as 'gs:///output'.
    """
    ...
    spark = SparkSession.builder.appName("My PyPi").getOrCreate()
    sc = spark.sparkContext

    hits = sc.parallelize(range(tries), partitions)\
        .map(calculate_hit)\
        .reduce(add)
    pi = 4.0 * hits / tries

    logger.info("%s tries and %s hits give a pi estimate of %s.",
                tries, hits, pi)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--output_uri', help="The URI where output is saved, such as 'gs://bucket/output'.")
